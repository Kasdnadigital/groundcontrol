...
! defined( 'WOOSC_VERSION' ) && define( 'WOOSC_VERSION', '4.2.5' );
			...
			class WPCleverWoosc {
				...
				function __construct() {
					self::$woosc_fields = array(
						...
						'mobile_or_fixed'   => esc_html__( 'Mobile or Fixed', 'woo-smart-compare' ),
						'power'   			=> esc_html__( 'Power', 'woo-smart-compare' ),
						'data_upload_speed' => esc_html__( 'Data Upload Speed', 'woo-smart-compare' ),
						'ip_rating' 		=> esc_html__( 'Ip Rating', 'woo-smart-compare' ),
						'airtime_provider'  => esc_html__( 'Airtime Provider', 'woo-smart-compare' ),
						'battery_life'  => esc_html__( 'Battery Life', 'woo-smart-compare' ),
						'talk_time'  => esc_html__( 'Talk Time', 'woo-smart-compare' ),
					);
				...
				function woosc_wp_enqueue_scripts() {
					...
					wp_enqueue_script( 'swal', 'https://cdn.jsdelivr.net/npm/sweetalert2@11.4.7/dist/sweetalert2.all.min.js', array(), false, true );
				...
				function woosc_get_table( $ajax = true ) {
					...
					if ( is_array( $woosc_products ) && ( count( $woosc_products ) > 0 ) ) {
						...
						array_push($saved_fields,"power","mobile_or_fixed","data_upload_speed","ip_rating","airtime_provider","battery_life","talk_time");
						...
						foreach ( $woosc_products as $woosc_product ) {
							...
							foreach ( $saved_fields as $saved_field ) {
								switch ( $saved_field ) {
									case 'image':
										if ( $link !== 'no' ) {
											$woosc_products_data[ $woosc_product ]['image'] = apply_filters( 'woosc_product_image', '<a ' . ( $link === 'yes_popup' ? 'class="woosq-link" data-id="' . $woosc_product . '" data-context="woosc"' : '' ) . ' href="' . $product->get_permalink() . '" draggable="false" ' . ( $link === 'yes_blank' ? 'target="_blank"' : '' ) . '><span class="p-img">' . $product->get_image( get_option( 'woosc_image_size', 'woosc-large' ), array( 'draggable' => 'false' ) ) . '</span><span class="p-title">'.wp_strip_all_tags( $product_name ).'</span>'.apply_filters( 'woosc_product_add_to_cart', do_shortcode( '[add_to_cart style="" show_price="false" id="' . $woosc_product . '"]' ), $product ).'</a>', $product );
										} else {
									...
									case 'availability':
										...
										break;
									case 'data_upload_speed':
										
										$dps = get_field("data_upload_speed", $product->get_id());
										if(!empty($dps))
										{ 
											$dbs_lbl = array();
											foreach ($dps as $key => $dps_val) {
												$dbs_lbl[] = $dps_val['label'];
											}
											
											$woosc_products_data[ $woosc_product ]['data_upload_speed'] = ucfirst(implode(", ",$dbs_lbl));
										}else
										{
											$woosc_products_data[ $woosc_product ]['data_upload_speed'] = " n/a ";
										}
										break;
									case 'power':
										
										$pwr = get_field("power", $product->get_id());
										if(!empty($pwr))
										{ 
											$pwr_lbl = array();
											foreach ($pwr as $pwr_val) {
												$pwr_lbl[] = $pwr_val['label'];
											}
											
											$woosc_products_data[ $woosc_product ]['power'] = ucfirst(implode(", ",$pwr_lbl));
										}else
										{
											$woosc_products_data[ $woosc_product ]['power'] = " n/a ";
										}
										break;	
									case 'mobile_or_fixed':
										
										$mf = get_field("mobile_or_fixed", $product->get_id());
										if(!empty($mf))
										{ 
											$woosc_products_data[ $woosc_product ]['mobile_or_fixed'] = ucfirst(implode(", ",$mf));
										}else
										{
											$woosc_products_data[ $woosc_product ]['mobile_or_fixed'] = " n/a ";
										}
										break;
									case 'ip_rating':
										
										$ipr = get_field("ip_rating", $product->get_id());
										if($ipr != '')
										{ 
											$woosc_products_data[ $woosc_product ]['ip_rating'] = ucfirst($ipr);
										}else
										{
											$woosc_products_data[ $woosc_product ]['ip_rating'] = " n/a ";
										}
										break;
									case 'airtime_provider':
										
										$arp = get_field("airtime_provider", $product->get_id());
										if(!empty($arp))
										{ 
											$arp_lbl = array();
											foreach ($arp as $arp_val) {
												$arp_lbl[] = $arp_val['label'];
											}
											
											$woosc_products_data[ $woosc_product ]['airtime_provider'] = ucfirst(implode(", ",$arp_lbl));
										}else
										{
											$woosc_products_data[ $woosc_product ]['airtime_provider'] = " n/a ";
										}
										break;
									case 'battery_life':
										
										$baty = get_field("battery_life", $product->get_id());
										if(!empty($baty))
										{ 
											$baty_lbl = array();
											foreach ($baty as $baty_val) {
												$baty_lbl[] = $baty_val['label'];
											}
											
											$woosc_products_data[ $woosc_product ]['battery_life'] = ucfirst(implode(", ",$baty_lbl));
										}else
										{
											$woosc_products_data[ $woosc_product ]['battery_life'] = " n/a ";
										}
										break;
									case 'talk_time':
										
										$talk = get_field("talk_time", $product->get_id());
										if(!empty($talk))
										{ 
											$talk_lbl = array();
											foreach ($talk as $talk_val) {
												$talk_lbl[] = $talk_val['label'];
											}
											
											$woosc_products_data[ $woosc_product ]['talk_time'] = ucfirst(implode(", ",$talk_lbl));
										}else
										{
											$woosc_products_data[ $woosc_product ]['talk_time'] = " n/a ";
										}
										break;						
									default:
						...
						$woosc_table .= '<table ' . ( $ajax ? 'id="woosc_table"' : '' ) . ' class="' . esc_attr( $woosc_table_class ) . '"><thead></thead>';
                        ...
                        <div class="woosc-inner">
									...
									<?php if ( 'yes' === get_option( 'woosc_close_button', 'yes' ) ) { ?>
                                        <a href="javascript:void(0);" id="woosc-table-close"
                                           ...
                                           aria-label="<?php echo esc_attr( self::woosc_localization( 'table_close', esc_html__( 'Close', 'woo-smart-compare' ) ) ); ?>"><span
                                                    class="woosc-table-close-icon">Close</span></a>
									<?php } ?>
                            ...
                            <div class="woosc-bar <?php echo esc_attr( get_option( 'woosc_bar_bubble', 'no' ) === 'yes' ? 'woosc-bar-bubble' : '' ); ?>">
                                ...
                                <div class="woosc-bar-btn woosc-bar-btn-text">
									...
									<?php echo apply_filters( 'woosc_bar_btn_text', self::woosc_localization( 'bar_button', esc_html__( 'Compare Products', 'woo-smart-compare' ) ) ); ?>
                                </div>
                            </div>
                        </div>
                    </div>
