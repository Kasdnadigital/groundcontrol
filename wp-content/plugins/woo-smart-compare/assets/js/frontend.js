...
(function($) {
  ...
  $(document).on('click touch', '.woosc-bar-remove', function() {
    ...
    woosc_close_table();
    var re = woosc_vars.remove_all;
    Swal.fire({
      title: woosc_vars.remove_all,
      icon: 'question',
      showCancelButton: true,
      confirmButtonText: 'Yes, remove!',
      cancelButtonColor: '#d33',
      confirmButtonColor: '#323552'
    }).then((result) => {
      if (re && result.isConfirmed) {
  ...
  $(document).on('click touch', function(e) {
    if ((
        ...
        ((woosc_vars.click_outside === 'yes_empty') &&
            (parseInt($('.woosc-bar').attr('data-count')) === 1))
    ) && (
        $(e.target).closest('.wpc_compare_count').length === 0
    ) && (
        $(e.target).closest('.woosc-popup').length === 0
    ) && (
        $(e.target).closest('.woosc-btn').length === 1
    ) && (
  ...
  function woosc_open_bar() {
    ...
    if($('.woosc-bar').attr('data-count') >= 2){
