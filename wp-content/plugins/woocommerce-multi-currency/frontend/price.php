...
//			add_filter( 'wmc_is_get_price_shipping', array( $this, 'apply_beauty_price_to_shipping' ) );
	...
	public function woocommerce_get_formatted_order_total( $formatted_total, $order ) {
		if ( ! $order || ! $this->show_approximate_price || ! in_array( 'total_thankyou', (array) self::$settings->get_param( 'approximate_position' ) ) ) {
			return $formatted_total;
		}

		$appro_price = $this->get_approximately_price( $order->get_total() );

		return self::$settings->get_param( 'approximately_priority' ) ? $appro_price . $formatted_total : $formatted_total . $appro_price;
	}
	...
	protected function set_cache( $price, $id, $key ) {
		...
		if ( $ajax === 'ppc-create-order' ) {
			...
			$price = self::convert_price_to_float( $price, $args );
		}
...
//			$fraction = $new_fraction = $price - $int;
	 ...
	 * @return float
	 */
	protected static function convert_price_to_float( $price, $args = array() ) {
		$args           = apply_filters(
			'wc_price_args',
			wp_parse_args(
				$args,
